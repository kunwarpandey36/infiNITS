...
function updateStudentData(filename = "src/lib/student-data.json") {
    fs.readFile(filename, 'utf8', (err, fileContent) => {
        ...
        const oldIds = studentsToUpdate.map(s => s.old_id);
        const updatedStudents = students.filter(s => !oldIds.includes(s.scholarId));

        // Add new student entries
        studentsToUpdate.forEach(studentUpdate => {
            const newStudent = {
                "scholarId": studentUpdate.new_id,
        ...
        fs.writeFile(filename, JSON.stringify(updatedStudents, null, 2), 'utf8', (err) => {
            if (err) {
                console.error(err);
            }
        });
