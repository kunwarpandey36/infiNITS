...
def update_student_data(filename="src/lib/student-data.json"):
    ...
    for student_update in students_to_update:
        old_id = student_update["old_id"]
        for branch in data["branches"]:
            data["branches"][branch]["students"] = [s for s in data["branches"][branch]["students"] if s["scholarId"] != old_id]

    # Add new student entries
    for student_update in students_to_update:
        new_id = student_update["new_id"]
        branch_code = new_id[2:4]
        ...
        if branch_name:
            new_student = {
                ...
                "cgpa": student_update["cgpa"],
                "sgpa_curr": student_update["sgpa"],
                "sgpa_prev": 0 
            }
            data["branches"][branch_name]["students"].append(new_student)
